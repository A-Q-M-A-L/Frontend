        ...
        <div class="backdrop-blur-xl shadow-2xl p-6 max-w-6xl w-[70rem] h-[80vh] rounded-xl">
            ...
            <div class="h-full overflow-y-auto">
                <table class="table-auto w-full border-separate border-spacing-2">
                    <thead>
                        <tr class="backdrop-blur-xl text-white shadow-[0_4px_6px_0_rgba(0,0,0,0.5)] text-center">
                    ...
                    <tbody>
                        ...
                        <tr v-if="filteredTasks.length === 0">
                            <td colspan="6"
                                class="border border-transparent font-sans font-medium text-xl px-4 py-2 text-center text-white pt-48">
                                No Tasks Assigned!
                            </td>
                        </tr>
                        <!-- Render tasks -->
                        <tr v-for="task in filteredTasks" :key="task._id"
                            class="shadow-[0_4px_6px_0_rgba(0,0,0,0.5)] text-white text-center">
                            <td class="border border-transparent px-4 py-2 max-w-xs">{{ task.title }}</td>
                            <td class="border border-transparent px-4 py-2 truncate max-w-xs">{{ task.description }}
                            </td>
                            <td class="border border-transparent px-4 py-2">{{ task.assignedTo.name.toUpperCase() }}
                            </td>
                            <td class="border border-transparent px-4 py-2">{{ task.status }}</td>
                            <td class="border border-transparent px-4 py-2">{{ new
                                Date(task.dueDate).toLocaleDateString() }}</td>
                        </tr>
...
export default {
    ...
    computed: {
        ...
        filteredTasks() {
            console.log("etioueiortuierutierutio");

            return this.AllTasks.filter(task => task.assignedTo._id === this.userId);
        }
    },
    mounted() {
        this.getAllTasks()
